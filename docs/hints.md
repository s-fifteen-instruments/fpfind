...
Optimization of peak finding often requires tuning the initial resolution `-R` (units of ns) and FFT buffer order `-q` (corresponding to the number of FFT bins $N=2^q$). The rate at which the timing uncertainty is reduced between iterations can be adjusted with `-Q` (default of 4 corresponds to 2 bits of accuracy per iteration).
